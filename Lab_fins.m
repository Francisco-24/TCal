...
x1 = [10, 36, 62];
y1 = [0.730769231, 0.615384615, 0.538461538];
...
ylim([0 1])
ylabel("$\frac{\theta}{\theta_b}$", 'Interpreter','latex', 'FontSize', 18)
xlabel("distance [mm]", 'FontSize', 12)
legend('theoretical temperature with h theo','theoretical temperature with h exp','experimental temperature','Location','southeast', 'FontSize', 12)

x = [0.0010, 0.0036, 0.0062]
y = [0.730769231, 0.615384615, 0.538461538];
fx=(cosh(m1.*(L-x)) + h_exp1./(m1.*k_al).*sinh(m1.*(L-x)))/(cosh(m1.*L) + h_exp1./(m1.*k_al).*sinh(m1*L))
error1=(fx-y)./fx
...
Prs2 = 0.707 + ((T_s2+273.15-300)/(350-300)*(0.700-0.707));
...
x = [0.0010, 0.0036, 0.0062]
y = [0.75, 0.625, 0.5625]
fx=(cosh(m2.*(L-x)) + h_exp2./(m2.*k_al).*sinh(m2.*(L-x)))/(cosh(m2.*L) + h_exp2./(m2.*k_al).*sinh(m2*L))
error2=(fx-y)./fx
